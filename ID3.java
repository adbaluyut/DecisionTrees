...
public class ID3 {
	...
	*/
	...
	StringTokenizer []st = new StringTokenizer[100];
   	...
   	*/
   	public int readData(String filename)  /* throws Exception */ {
		...
		attributeNames = new String[numAttributes];
			...
			st[index] = new StringTokenizer(input);
			index++;
	...
	*/
	...
	// assignParty parses the data and outputs the results to a .txt file
	public static void assignParty(StringTokenizer []st, String fileName) {
		String [][]tokenArr = new String[100][17];
		for (int i = 0; i < st.length; i++){
			int j = 0;
			while (st[i].hasMoreTokens()) {
				String temp = st[i].nextToken();
		...
		try {
			...
			for (int i = 0; i < tokenArr.length; i++) {		
				String Physicianfeefreeze = tokenArr[i][3];
				String Adoptionofthebudgetresolution = tokenArr[i][2];
				String Antisatellitetestban = tokenArr[i][6];	
		...
		try{
			...
			// File f = new File("testSetResults.txt");
			File f2 = new File(fileName);
			Scanner scnr = new Scanner(f2);
			// compare decision tree to file
			for (int i = 0; i < tokenArr.length; i++)
			{
				if (tokenArr[i][tokenArr[i].length - 1].equals(scnr.nextLine())) {
					sum++;
				}
			}
			scnr.close();
	...
	//	}
		...
		int status2 = me2.readData("test-house-votes-1984.txt");
		if (status <= 0 || status2 <= 0) return;

		me.createDecisionTree();

		assignParty(me.st, "TrainingSetResults.txt");
		assignParty(me2.st, "TestSetResults.txt");
